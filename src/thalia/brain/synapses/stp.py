...
class STPConfig:
    ...
    def steady_state_utilization(self, rate_hz: float) -> float:
        ...
        Typical values at biologically relevant rates:

        +---------------------+--------+-------+
        | ``STPType``         | Rate   | u_eff |
        +=====================+========+=======+
        | DEPRESSING          | 45 Hz  | 0.027 |
        | DEPRESSING          | 10 Hz  | 0.162 |
        | THALAMOCORTICAL*    | 30 Hz  | 0.044 |
        | FACILITATING        |  5 Hz  | 0.064 |
        | CORTICOSTRIATAL     | 10 Hz  | 0.237 |
        +---------------------+--------+-------+

        \\* Approximate; uses U=0.4, τ_d=700ms, τ_f=30ms.
