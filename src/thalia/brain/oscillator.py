...
from thalia.constants import DEFAULT_DT_MS, MS_PER_SECOND, TAU
...
class OscillatorManager:
    ...
    def __init__(
        ...
        **Theta (4-12 Hz)** is DISABLED by default - emerges from medial septum
        pacemaker → hippocampal OLM interneuron circuit. This provides biologically
        accurate theta with natural phase-locking and encoding/retrieval dynamics.

        **Gamma (30-80 Hz)** is DISABLED by default - emerges from L6→TRN→Thalamus
        feedback loop (~25ms = 40 Hz). Two gamma frequencies naturally arise from
        L6a-TRN-relay (~40Hz) and L6b-relay (~60Hz) circuits.

        Enable either explicitly if needed via enable_oscillator('theta'/'gamma', True).
        ...
        self._enabled: Dict[str, bool] = {
            "delta": True,
            "theta": False,  # Disabled: Emerges from medial septum → OLM circuit
            "alpha": True,
            "beta": True,
            "gamma": False,  # Disabled: Emerges from L6→TRN loop (~25ms)
            "ripple": True,
        }
