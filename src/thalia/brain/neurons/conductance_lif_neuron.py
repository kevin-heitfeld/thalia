...
class ConductanceLIF(nn.Module):
    ...
    def __init__(self, n_neurons: int, config: ConductanceLIFConfig, device: Union[str, torch.device] = GlobalConfig.DEFAULT_DEVICE):
    ...
    def forward(
        ...
        **RNG Independence** (Critical for Biological Accuracy):
        ...
        g_nmda_effective = self.g_nmda * mg_block  # Effective (voltage-gated) NMDA conductance
        ...
        if config.enable_t_channels:
            ...
            h_T_inf = 1.0 / (1.0 + torch.exp((self.V_soma - config.V_half_h_T) / config.k_h_T))
            ...
            g_T_eff = config.g_T * m_T_inf * self.h_T

            # Add T-conductance to membrane dynamics
            g_total = g_total + g_T_eff
            V_inf_numerator = V_inf_numerator + g_T_eff * config.E_Ca
        ...
        if config.enable_ih:
            h_inf = 1.0 / (1.0 + torch.exp((self.V_soma - config.V_half_h) / config.k_h))
            ...
            g_ih = config.g_h_max * self.h_gate  # [n_neurons]
            g_total = g_total + g_ih
            V_inf_numerator = V_inf_numerator + g_ih * config.E_h
...
class TwoCompartmentLIF(nn.Module):
    ...
    *Critical*: NMDA Mg²⁺ block for **apical** NMDA is gated by ``V_dend`` (not
    ...
    def __init__(self, n_neurons: int, config: TwoCompartmentLIFConfig, device: Union[str, torch.device] = GlobalConfig.DEFAULT_DEVICE):
    ...
    def forward(
        ...
        **CRITICAL**: All conductance inputs must be non-negative.  They represent
        ...
        g_nmda_b_effective = self.g_nmda_basal * self._compute_nmda_block(self.V_soma)

        # Mg²⁺ block at DENDRITE for apical NMDA (biologically correct: block at synapse site)
        g_nmda_a_effective = self.g_nmda_apical * self._compute_nmda_block(self.V_dend)
