...
class EmergentGoalSystem:
    ...
    def get_state_dict(self) -> dict:
        ...
        Returns:
            Dictionary with transition weights, value weights, and tags
        """
        return {
            "transition_weights": self.transition_weights,
            "value_weights": self.value_weights,
            "goal_tags": self.goal_tags,
            "n_wm_neurons": self.n_wm_neurons,
    ...
    def load_state_dict(self, state_dict: dict) -> None:
        ...
        """
        self.transition_weights = state_dict["transition_weights"].to(self.device)
        self.value_weights = state_dict["value_weights"].to(self.device)
        self.goal_tags = state_dict["goal_tags"].to(self.device)

        # Verify dimensions match
        assert self.n_wm_neurons == state_dict["n_wm_neurons"]
        assert self.n_abstract == state_dict["n_abstract"]
        assert self.n_concrete == state_dict["n_concrete"]

    def reset_tags(self) -> None:
        """Reset synaptic tags to zero.

        Useful for starting new episodes or tasks.
        """
        self.goal_tags.zero_()
