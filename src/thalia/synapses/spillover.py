...
class SpilloverConfig:
    ...
    Attributes:
        enabled: Whether to apply spillover augmentation
...
class SpilloverTransmission:
    ...
    Usage:
        >>> weights = torch.randn(100, 50)  # [n_post, n_pre]
        >>> config = SpilloverConfig(enabled=True, strength=0.15)
        >>> spillover = SpilloverTransmission(weights, config)
        >>> effective_weights = spillover.get_effective_weights()  # Use in forward pass
    """
    ...
    def __init__(
        ...
        if config.enabled:
            ...
            self.W_spillover = torch.zeros_like(self.W_direct)
            self.W_effective = self.W_direct
