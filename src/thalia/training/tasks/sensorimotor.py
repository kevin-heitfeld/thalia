...
from thalia.stimuli.stimulus_utils import (
...
class MotorControlConfig:
    ...
    noise_level: float = 0.1  # Noise in proprioceptive feedback
...
class MotorControlTask:
    ...
    **Example Usage**:
    ...
    def compute_reward(self, output: Dict[str, torch.Tensor], target: Optional[torch.Tensor] = None) -> float:
...
class ManipulationTask:
    ...
    def compute_reward(self, output: Dict[str, torch.Tensor], target_action: Optional[str] = None) -> float:
...
class SensorimotorTaskLoader:
    ...
    def compute_reward(
        self, output: Dict[str, torch.Tensor], task_data: Optional[Dict[str, Any]] = None
    ) -> float:
