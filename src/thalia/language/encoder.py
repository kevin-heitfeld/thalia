...
class EncodingType(Enum):
    ...
    PHASE = "phase"         # Spike phase relative to oscillation
    BURST = "burst"         # Burst patterns encode value
...
class SpikeEncoderConfig:
    ...
    Attributes:
        ...
        n_neurons: Number of output neurons for spike representation
        sparsity: Target fraction of active neurons (0.02 = 2%)
        n_timesteps: Number of timesteps for temporal encoding
        encoding_type: How to encode information in spikes
        
        # Temporal parameters
        dt_ms: Simulation timestep
        spike_threshold: Threshold for converting activations to spikes
        ...
        theta_frequency_hz: Theta oscillation frequency for phase coding
    ...
    def theta_period_ms(self) -> float:
        """Period of theta oscillation in ms."""
...
class SpikeEncoder(nn.Module):
    ...
    def __init__(self, config: SpikeEncoderConfig):
        ...
        # Temporal spike generator
        self._init_temporal_encoder()
        
        # Phase reference for position coding
        self.register_buffer(
            "theta_phase",
            torch.tensor(0.0),
        )
        self.phase_increment = 2 * math.pi * config.dt_ms / config.theta_period_ms
    
    def _init_temporal_encoder(self) -> None:
        """Initialize components for temporal encoding."""
        config = self.config
        
        if config.encoding_type == EncodingType.RATE:
            # Rate coding: Convert activations to spike probabilities
            self.rate_scale = nn.Parameter(torch.ones(config.n_neurons))
            
        elif config.encoding_type == EncodingType.TEMPORAL:
            # Temporal coding: Rank-order coding based on activation strength
            self.temporal_scale = nn.Parameter(torch.ones(config.n_neurons) * 10.0)
            
        elif config.encoding_type == EncodingType.PHASE:
            # Phase coding: Spike timing relative to theta
            self.phase_offset = nn.Parameter(torch.zeros(config.n_neurons))
            
        elif config.encoding_type == EncodingType.BURST:
            # Burst coding: Number of spikes in burst encodes value
            self.burst_threshold = nn.Parameter(torch.ones(config.n_neurons) * 0.5)
    ...
    def forward(
        ...
        # Step 1: Get SDR patterns
        sdr = self.sdr(token_ids)  # [batch, seq_len, n_neurons]
        
        # Step 2: Convert to temporal spike trains
        spikes = self._generate_spike_train(sdr)  # [batch, seq, timesteps, neurons]
    ...
    def _generate_spike_train(
        ...
        if self.config.encoding_type == EncodingType.SDR:
        ...
        elif self.config.encoding_type == EncodingType.RATE:
        ...
        elif self.config.encoding_type == EncodingType.TEMPORAL:
        ...
        elif self.config.encoding_type == EncodingType.PHASE:
        ...
        elif self.config.encoding_type == EncodingType.BURST:
