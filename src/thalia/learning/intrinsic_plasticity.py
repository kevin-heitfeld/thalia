...
Unlike synaptic plasticity (which modifies connections), intrinsic plasticity
modifies the neuron's internal properties:
...
2. HOMEOSTATIC FUNCTION: Maintains stable firing rates across the population,
   even as synaptic weights change through learning.

3. MEMORY FUNCTION: Persistent threshold changes encode long-term
   excitability preferences (a form of memory).

Biological Basis:
=================
- Voltage-gated ion channel expression changes with activity
- CREB-dependent gene expression modifies intrinsic excitability
- After-hyperpolarization (AHP) currents adapt with activity
...
Where:
- τ_θ is the adaptation time constant (slow, ~10-100 seconds)
- target_rate is the desired firing rate
- rate_i is a running average of neuron i's firing rate
- η is the learning rate for threshold adaptation
