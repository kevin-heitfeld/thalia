...
class EligibilityTraceManager:
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        Returns:
            State dictionary with traces and configuration
        """
        return {
    ...
    def load_state(self, state: Dict[str, Any]) -> None:
        ...
        """
        if state["n_input"] != self.n_input:
            raise ValueError(f"Input dimension mismatch: {state['n_input']} != {self.n_input}")
        if state["n_output"] != self.n_output:
            raise ValueError(f"Output dimension mismatch: {state['n_output']} != {self.n_output}")

        self.input_trace = state["input_trace"].to(self.device)
        self.output_trace = state["output_trace"].to(self.device)
        self.eligibility = state["eligibility"].to(self.device)
