"""
Tests for brain surgery tools on DynamicBrain.

Tests lesion, ablation, and growth operations on trained brains.
This is an exact copy of test_surgery.py but using DynamicBrain instead of EventDrivenBrain.
"""
...
def test_lesion_region_silences_region(test_brain):
    ...
    initial_weights = {
        ...
        for name, param in test_brain.hippocampus.named_parameters()
    }
    ...
    assert test_brain.hippocampus.plasticity_enabled is False
...
def test_lesion_region_alternate_names(test_brain):
    ...
    assert test_brain.pfc.plasticity_enabled is False
...
def test_partial_lesion(test_brain):
    ...
    has_nonzero = False
    for param in test_brain.cortex.parameters():
...
def test_temporary_lesion(test_brain):
    ...
    # Save initial state
    initial_weights = list(test_brain.striatum.parameters())[0].data.clone()

    # Temporary lesion
    with temporary_lesion(test_brain, "striatum"):
        # Inside context: lesioned
        lesioned_weights = list(test_brain.striatum.parameters())[0].data.clone()
        assert torch.all(lesioned_weights == 0.0)

    # Outside context: restored
    restored_weights = list(test_brain.striatum.parameters())[0].data.clone()
...
def test_restore_region(test_brain):
    ...
    initial_weights = {
        name: param.data.clone()
        for name, param in test_brain.cerebellum.named_parameters()
    }
...
def test_ablate_pathway_alternate_names(test_brain):
    ...
    # DynamicBrain uses full names, so test with pathway that exists
    ablate_pathway(test_brain, "cortex_to_hippocampus")
    pathway = test_brain.pathways["cortex_to_hippocampus"]
    assert pathway.plasticity_enabled is False
...
def test_freeze_region(test_brain):
    ...
    # Check plasticity disabled
    assert test_brain.cortex.plasticity_enabled is False
...
def test_unfreeze_region(test_brain):
    ...
    assert test_brain.hippocampus.plasticity_enabled is True

    # Check parameters unfrozen
    for param in test_brain.hippocampus.parameters():
