...
class TestElasticTensorMetadata:
    ...
    def test_checkpoint_includes_capacity_metadata(self, striatum_small, tmp_path):
        ...
        assert (
            0 < neuron_state["n_neurons_active"] <= 100
    ...
    def test_checkpoint_with_population_coding(
        self, base_config, base_sizes_population, device, tmp_path
    ):
    ...
    def test_growth_works_with_population_coding(
        self, base_config, base_sizes_population, device, tmp_path
    ):
        ...
        assert (
            region2.n_actions == region.n_actions
        ), "Restored region should have same action count as original"
        assert (
            region2.n_neurons_active == region.n_neurons_active
...
class TestLoadingSmallerCheckpoint:
    ...
    def test_load_smaller_checkpoint_succeeds(self, striatum_small, striatum_large, tmp_path):
        ...
        assert (
            striatum_large.n_neurons_active == expected_neurons
        ), f"Large brain should remain {expected_neurons} neurons (doesn't shrink)"
        assert (
            striatum_large.n_actions == expected_actions
    ...
    def test_partial_state_restoration(self, striatum_small, striatum_large, tmp_path):
        ...
        striatum_small.d1_pathway.neurons.membrane[:5] = torch.arange(
            5, dtype=torch.float32, device=striatum_small.device
        )
        ...
        assert (
            striatum_large.d1_pathway.neurons.membrane.shape[0] == striatum_large.d1_size
        )  # Pathways keep size
...
class TestLoadingLargerCheckpoint:
    ...
    def test_load_larger_checkpoint_triggers_growth(self, striatum_small, striatum_large, tmp_path):
        ...
        assert (
            striatum_small.n_neurons_active == initial_small_size
        ...
        assert (
            striatum_small.n_neurons_active == expected_grown_size
    ...
    def test_auto_growth_preserves_state(self, striatum_small, striatum_large, tmp_path):
        ...
        striatum_large.d1_pathway.neurons.membrane[:10] = torch.arange(
            10, dtype=torch.float32, device=striatum_large.device
        )
    ...
    def test_growth_exceeding_capacity_expands(self, base_config, device, tmp_path):
        ...
        assert (
            small_striatum.n_neurons_active == expected_size
        ), f"Should grow to {expected_size} neurons to match checkpoint"
        assert (
            small_striatum.n_neurons_capacity >= expected_size
...
class TestReservedSpaceUtilization:
    ...
    @pytest.mark.parametrize(
        "growth_amount,expect_reallocation,description",
        [
    ...
    def test_growth_with_various_amounts(
        self, striatum_small, growth_amount, expect_reallocation, description
    ):
        ...
        assert (
            striatum_small.n_neurons_active == expected_active
        ...
        assert (
            striatum_small.d1_pathway.neurons.n_neurons >= expected_active // 2 - 1
        ), f"D1 pathway should have ~{expected_active // 2} neurons"
        assert (
            striatum_small.d2_pathway.neurons.n_neurons >= expected_active // 2 - 1
        ...
        if expect_reallocation:
            ...
            assert (
                striatum_small.n_neurons_capacity > initial_capacity
            ), f"Capacity should increase for {description}"
            expected_capacity = int(expected_active * 1.5)
            assert (
                striatum_small.n_neurons_capacity >= expected_capacity
            ...
            assert (
                striatum_small.n_neurons_capacity >= expected_active
    ...
    def test_checkpoint_after_growth_has_correct_capacity(self, striatum_small, tmp_path):
        ...
        assert (
            loaded["neuron_state"]["n_neurons_active"] == n_grown_neurons
        ), f"Metadata should show {n_grown_neurons} active neurons after growth"
        assert (
            loaded["neuron_state"]["n_neurons_capacity"] >= n_grown_neurons
...
class TestEdgeCases:
    ...
    def test_save_load_round_trip(self, striatum_small, tmp_path):
        ...
        assert torch.allclose(striatum_small.d1_pathway.neurons.membrane, original_membrane)
...
class TestPerformance:
    ...
    def test_memory_usage_tracks_capacity(self, base_config, device):
        ...
        assert (
            region.n_neurons_capacity >= region.n_neurons_active
        ...
        assert (
            actual_neurons_allocated == expected_per_pathway
