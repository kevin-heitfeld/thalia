...
class TestEquivalenceWithEventDrivenBrain:
    """Test that port-based DynamicBrain achieves equivalence with EventDrivenBrain."""

    def test_equivalent_topology_produces_matching_sizes(self, device):
        """Test that port-based topology matches EventDrivenBrain sizes."""
        ...
        # Build DynamicBrain with port-based routing
        builder = BrainBuilder(config.global_)

        sizes = config.brain.sizes

        builder.add_component("thalamus", "thalamus", n_input=sizes.input_size, n_output=sizes.thalamus_size)
        ...
        # EventDrivenBrain-equivalent topology with ports
        builder.connect("thalamus", "cortex", target_port="feedforward")
        builder.connect("cortex", "hippocampus", source_port="l23", target_port="cortical")
        builder.connect("thalamus", "hippocampus", target_port="ec_l3")  # Direct entorhinal
        builder.connect("cortex", "pfc", source_port="l23")
        builder.connect("hippocampus", "pfc")
        builder.connect("hippocampus", "striatum", target_port="hippocampal")
        builder.connect("cortex", "striatum", source_port="l5", target_port="cortical")
        builder.connect("pfc", "striatum")  # PFC counts as feedforward input (not separate modulation)
        builder.connect("striatum", "pfc")  # Bidirectional
        builder.connect("striatum", "cerebellum")

        dynamic_brain = builder.build()
        ...
        # Cortex sizes should match
        assert dynamic_cortex.config.n_input == event_cortex.config.n_input
        assert dynamic_cortex.config.n_output == event_cortex.config.n_output
        assert dynamic_cortex.l23_size == event_cortex.l23_size
        assert dynamic_cortex.l5_size == event_cortex.l5_size

        # Hippocampus sizes should match
        dynamic_hippo = dynamic_brain.components["hippocampus"]
        event_hippo = event_brain.adapters["hippocampus"].impl
        assert dynamic_hippo.config.n_input == event_hippo.config.n_input
        assert dynamic_hippo.config.n_output == event_hippo.config.n_output
        assert dynamic_hippo.config.ec_l3_input_size == event_hippo.config.ec_l3_input_size

        # Striatum sizes should match
        dynamic_striatum = dynamic_brain.components["striatum"]
        event_striatum = event_brain.adapters["striatum"].impl
        assert dynamic_striatum.config.n_input == event_striatum.config.n_input
