...
class TestVTADopamineSystem:
    ...
    def test_phasic_dip_from_negative_rpe(self, vta):
        ...
        vta.deliver_reward(reward=0.0, expected_reward=1.0)
    ...
    def test_adaptive_rpe_normalization(self, vta):
        ...
        # First reward → large normalized RPE
        vta.deliver_reward(reward=1.0, expected_reward=0.0)
        first_rpe = vta._last_normalized_rpe
        ...
        later_rpe = vta._last_normalized_rpe

        # Later RPE should be smaller (adapted to large rewards)
        assert later_rpe < first_rpe
    ...
    def test_reset_state(self, vta):
        ...
        assert vta._global_dopamine != vta.config.baseline_dopamine

        # Reset
        vta.reset_state()

        assert vta._global_dopamine == vta.config.baseline_dopamine
        assert vta._tonic_dopamine == vta.config.baseline_dopamine
        assert vta._phasic_dopamine == 0.0
    ...
    def test_health_check_detects_runaway(self, vta):
        ...
        # Force runaway dopamine
        vta._global_dopamine = 5.0

        health = vta.check_health()

        assert health["is_healthy"] is False
        assert any("dopamine too high" in issue.lower() for issue in health["issues"])

    def test_health_check_detects_negative(self, vta):
        """Test health check detects negative dopamine."""
        # Force negative dopamine
        vta._global_dopamine = -0.5
    ...
    def test_zero_rpe_no_phasic(self, vta):
        ...
        vta.deliver_reward(reward=0.5, expected_reward=0.5)
    ...
    def test_get_diagnostics(self, vta):
        """Test diagnostic information."""
        vta.update(dt_ms=1.0, intrinsic_reward=0.2)
        vta.deliver_reward(reward=1.0, expected_reward=0.3)

        diag = vta.get_diagnostics()

        assert "global_dopamine" in diag
        assert "tonic_dopamine" in diag
        assert "phasic_dopamine" in diag
        assert "last_rpe" in diag
        assert "rpe_std" in diag
    ...
    def test_multiple_rewards_per_timestep(self, vta):
        ...
        vta.deliver_reward(reward=0.5, expected_reward=0.0)
        vta.deliver_reward(reward=0.5, expected_reward=0.0)
    ...
    def test_clipping_prevents_overflow(self, vta):
        ...
        for _ in range(100):
            vta.deliver_reward(reward=10.0, expected_reward=0.0)
            vta.update(dt_ms=1.0, intrinsic_reward=1.0)

        # Should be clipped
        da = vta.get_global_dopamine()
        assert da >= 0.0
        assert da <= 2.0  # Reasonable upper bound

    def test_config_parameters_affect_behavior(self):
        """Test different configs produce different behavior."""
        # Fast decay config
        fast_config = VTAConfig(dopamine_decay_per_ms=0.9)  # τ~10ms
        fast_vta = VTADopamineSystem(fast_config)

        # Slow decay config
        slow_config = VTAConfig(dopamine_decay_per_ms=0.999)  # τ~1000ms
        slow_vta = VTADopamineSystem(slow_config)

        # Both get same reward
        fast_vta.deliver_reward(reward=1.0, expected_reward=0.0)
        slow_vta.deliver_reward(reward=1.0, expected_reward=0.0)
