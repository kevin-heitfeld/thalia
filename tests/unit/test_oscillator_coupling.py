...
class TestOscillatorCoupling:
    ...
    def test_coupling_creation(self):
        ...
        coupling = OscillatorCoupling(
            slow_oscillator='theta',
            fast_oscillator='gamma',
...
class TestCrossFrequencyCoupling:
    ...
    def test_default_couplings_all_five(self):
        """Test that default manager includes all 5 biologically-motivated couplings."""
        manager = OscillatorManager()
        
        # Should have 5 default couplings
        assert len(manager.couplings) == 5
        
        # Extract coupling pairs
        coupling_pairs = [(c.slow_oscillator, c.fast_oscillator) for c in manager.couplings]
        
        # Verify all expected couplings are present
        assert ('theta', 'gamma') in coupling_pairs  # Working memory
        assert ('beta', 'gamma') in coupling_pairs   # Motor timing
        assert ('delta', 'theta') in coupling_pairs  # Sleep consolidation
        assert ('alpha', 'gamma') in coupling_pairs  # Attention gating
        assert ('theta', 'beta') in coupling_pairs   # Working memory-action
    ...
    def test_default_theta_gamma_coupling(self):
        ...
        for c in manager.couplings:
            if c.slow_oscillator == 'theta' and c.fast_oscillator == 'gamma':
                theta_gamma = c
                break
        
        assert theta_gamma is not None
        assert theta_gamma.coupling_strength == 0.8
        assert theta_gamma.min_amplitude == 0.2
        assert theta_gamma.modulation_type == 'cosine'
    
    def test_custom_couplings(self):
        """Test creating manager with custom couplings."""
        couplings = [
            OscillatorCoupling('theta', 'gamma', 0.9, 0.1),
            OscillatorCoupling('delta', 'theta', 0.7, 0.3),
        ]
        manager = OscillatorManager(couplings=couplings)
        
        assert len(manager.couplings) == 2
        assert manager.couplings[0].slow_oscillator == 'theta'
        assert manager.couplings[1].slow_oscillator == 'delta'
...
class TestMultipleCouplings:
    ...
    def test_multiple_couplings_applied(self):
        ...
        couplings = [
            OscillatorCoupling('theta', 'gamma', 0.8, 0.2),
            OscillatorCoupling('delta', 'theta', 0.6, 0.4),
            OscillatorCoupling('alpha', 'beta', 0.5, 0.5),
        ]
        manager = OscillatorManager(couplings=couplings)
    ...
    def test_no_interference_between_couplings(self):
        ...
        couplings = [
            OscillatorCoupling('theta', 'gamma', 0.8, 0.2),
            OscillatorCoupling('delta', 'alpha', 0.6, 0.3),
        ]
        manager = OscillatorManager(couplings=couplings)
...
class TestBiologicalAccuracy:
    ...
    def test_delta_theta_nrem_pattern(self):
        ...
        coupling = OscillatorCoupling('delta', 'theta', 0.7, 0.1)
        manager = OscillatorManager(
            delta_freq=2.0,
...
class TestNewCouplings:
    ...
    def test_beta_gamma_motor_timing(self):
        ...
        # Find beta-gamma coupling
        beta_gamma = None
        for c in manager.couplings:
            if c.slow_oscillator == 'beta' and c.fast_oscillator == 'gamma':
                beta_gamma = c
                break
        
        assert beta_gamma is not None, "Beta-gamma coupling should exist"
        assert beta_gamma.coupling_strength == 0.6
        assert beta_gamma.min_amplitude == 0.3
        assert beta_gamma.modulation_type == 'cosine'
        ...
        # With coupling_strength=0.6, min_amplitude=0.3:
        # At phase=0 (cos=1): 0.3 + (1-0.3)*1.0*0.6 = 0.72
        # At phase=π (cos=-1): 0.3 + (1-0.3)*0.0*0.6 = 0.3
        assert 0.7 <= gamma_amp_high <= 0.75
        assert 0.29 <= gamma_amp_low <= 0.31
    
    def test_delta_theta_sleep_consolidation(self):
        """Test delta-theta coupling for sleep consolidation."""
        manager = OscillatorManager()
        
        # Find delta-theta coupling
        delta_theta = None
        for c in manager.couplings:
            if c.slow_oscillator == 'delta' and c.fast_oscillator == 'theta':
                delta_theta = c
                break
        
        assert delta_theta is not None, "Delta-theta coupling should exist"
        assert delta_theta.coupling_strength == 0.7
        assert delta_theta.min_amplitude == 0.1
        assert delta_theta.modulation_type == 'cosine'
    ...
    def test_alpha_gamma_attention_gating(self):
        ...
        # Find alpha-gamma coupling
        alpha_gamma = None
        for c in manager.couplings:
            if c.slow_oscillator == 'alpha' and c.fast_oscillator == 'gamma':
                alpha_gamma = c
                break
        
        assert alpha_gamma is not None, "Alpha-gamma coupling should exist"
        assert alpha_gamma.coupling_strength == 0.5
        assert alpha_gamma.min_amplitude == 0.4
        assert alpha_gamma.modulation_type == 'sine'  # Inverse coupling
    ...
    def test_theta_beta_working_memory_action(self):
        ...
        # Find theta-beta coupling
        theta_beta = None
        for c in manager.couplings:
            if c.slow_oscillator == 'theta' and c.fast_oscillator == 'beta':
                theta_beta = c
                break
        
        assert theta_beta is not None, "Theta-beta coupling should exist"
        assert theta_beta.coupling_strength == 0.4
        assert theta_beta.min_amplitude == 0.5
        assert theta_beta.modulation_type == 'cosine'
        ...
        # With coupling_strength=0.4, min_amplitude=0.5:
        # At phase=0 (cos=1): 0.5 + (1-0.5)*1.0*0.4 = 0.7
        # At phase=π (cos=-1): 0.5 + (1-0.5)*0.0*0.4 = 0.5
        assert 0.69 <= beta_amp_high <= 0.71
        assert 0.49 <= beta_amp_low <= 0.51
