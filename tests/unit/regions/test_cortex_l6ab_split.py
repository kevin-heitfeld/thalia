...
class TestL6abSplit:
    ...
    def test_l6ab_weights(self, cortex_config_l6ab, cortex_sizes, device):
        ...
        # L2/3 → L6a weights
        assert cortex.synaptic_weights["l23_l6a"].shape == (
            l6a_size,
            l23_size,
        ), f"L6a weight shape should be ({l6a_size}, {l23_size}), got {cortex.synaptic_weights['l23_l6a'].shape}"
        assert not torch.isnan(
            cortex.synaptic_weights["l23_l6a"]
        ).any(), "L6a weights contain NaN values"
        assert not torch.isinf(
            cortex.synaptic_weights["l23_l6a"]
        ).any(), "L6a weights contain Inf values"

        # L2/3 → L6b weights
        assert cortex.synaptic_weights["l23_l6b"].shape == (
            l6b_size,
            l23_size,
        ), f"L6b weight shape should be ({l6b_size}, {l23_size}), got {cortex.synaptic_weights['l23_l6b'].shape}"
        assert not torch.isnan(
            cortex.synaptic_weights["l23_l6b"]
        ).any(), "L6b weights contain NaN values"
        assert not torch.isinf(
            cortex.synaptic_weights["l23_l6b"]
        ).any(), "L6b weights contain Inf values"
    ...
    def test_l6ab_port_routing(self, cortex_config_l6ab, cortex_sizes, device):
        ...
        assert l6a_output.shape == (
            l6a_size,
        ), f"L6a output shape should be ({l6a_size},), got {l6a_output.shape}"
        assert (
            l6a_output.dtype == torch.bool
        ), f"L6a output should be bool spikes, got {l6a_output.dtype}"
        assert not torch.isnan(l6a_output.float()).any(), "L6a output contains NaN values"
        ...
        assert l6b_output.shape == (
            l6b_size,
        ), f"L6b output shape should be ({l6b_size},), got {l6b_output.shape}"
        assert (
            l6b_output.dtype == torch.bool
        ), f"L6b output should be bool spikes, got {l6b_output.dtype}"
        assert not torch.isnan(l6b_output.float()).any(), "L6b output contains NaN values"
