...
def test_gap_junctions_enabled_by_default(default_config):
...
def test_gap_junction_creates_coupling(default_config, device):
...
def test_fsi_inhibition_effect(default_config, device):
...
def test_gap_junction_state_management(default_config, device):
...
def test_gap_junction_state_serialization(default_config, device):
    ...
    striatum2 = Striatum(default_config)
...
def test_gap_junction_uses_fsi_weights(default_config, device):
...
def test_gap_junction_integration_with_beta(default_config, device):
